...
/*   v1.1
 *   Tempo and Velocity Pots now work on A6 & A7
 *   A6 change instances analogRead(0) to analogRead(6)
 *   A7 change instances analogRead(1) to analogRead(7)
 *    
 *    Arp Test V1
 *   
...
#define STAT1    A6 //7 Velocity **remember, also change instances analogRead(0) to analogRead(6)
#define STAT2    A7 //6 Tempo **remember, change instances analogRead(1) to analogRead(7)
...
void HandleClock () {
  ...
  if (clockTick >= (int)map(analogRead(7),0,1023,1,4)*6 + 1) { // changed analogRead(1) to analogRead(7(
    handleTick(tick);
    clockTick = 1;
  }
...
void loop() {
  ...
  if (!clockSync) {  
    ...
    tempo = 6000 / ((127-analogRead(7)/8) + 2);   // changed analogRead(1) to analogRead(7)
    handleTick(tick);
  }
  ...
  int velocity() {
    int velocity = 127 - analogRead(6)/8; // changed analogRead(0) to analogRead(6)
